...
void main() {
  group('RoutePreviewService Pure Unit Tests', () {
    ...
    test('Computes accurate bounding box, distance, and walking time', () {
      ...
      expect(
        preview.statistics.estimatedWalkingTimeMinutes,
        8,
      ); // (500 / 70).ceil() = 8
    ...
    test(
      'Computes cross-floor route summary and connector details correctly',
      () {
        final nodesFirst = [
          const NavigationNodeEntity(
            id: 'N10',
        ...
        final routeCrossFloor = NavigationRouteEntity(
          ...
          segments: [
            ...
            RouteSegmentEntity(
              floorId: 'transition',
              nodes: [nodesGround.last, nodesFirst.first],
            ),
  ...
  group('MapBloc Route Preview Mode Integration Tests', () {
    ...
    test(
      'CalculateRoute enters preview mode without starting positioning simulation',
      () async {
    ...
    test(
      'StartNavigation transitions out of preview mode and starts positioning simulation',
      () async {
        mapBloc.add(const LoadMap());
        await expectLater(mapBloc.stream, emitsThrough(isA<MapLoaded>()));
