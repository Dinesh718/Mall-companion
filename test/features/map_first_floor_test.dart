...
void main() {
  ...
  group('First Floor Integration and Selector Tests', () {
    ...
    test(
      'vertical connectors for Ground and First floors are correctly registered and linked',
      () async {
        final mapData = await repository.getMapData();
        ...
        final gEscalator = groundFloor.connectors.firstWhere(
          (c) => c.id == 'escalator_1_g',
        );
        ...
        final fEscalator = firstFloor.connectors.firstWhere(
          (c) => c.id == 'escalator_1_f',
        );
        ...
        final gNode = groundFloor.navigationGraph.nodes.firstWhere(
          (n) => n.id == 'H009',
        );
        expect(gNode.x, 1300.0);
        expect(gNode.y, 440.0);

        final fNode = firstFloor.navigationGraph.nodes.firstWhere(
          (n) => n.id == 'H109',
        );
    ...
    test(
      'cross-floor route generation between Ground Floor shop and First Floor shop produces correct 3 segments',
      () async {
    ...
    test(
      'automatic floor transition triggers when position stream crosses connector and changes floorId',
      () async {
        mapBloc.add(const LoadMap());
        await expectLater(mapBloc.stream, emitsThrough(isA<MapLoaded>()));
        ...
        mapBloc.add(
          const CalculateRoute(startNodeId: 'E026', endNodeId: 'E117'),
        );
        await expectLater(
          mapBloc.stream,
          emitsThrough(
    ...
    test(
      'manual floor selection during active navigation preserves session and route',
      () async {
        mapBloc.add(const LoadMap());
        await expectLater(mapBloc.stream, emitsThrough(isA<MapLoaded>()));
        ...
        expect(state.navigationSession, isNotNull);
        expect(
          state.navigationSession?.currentSegmentIndex,
          0,
        ); // Still starting segment
      },
    );

    test(
      'cross-floor route generation from First Floor shop to Ground Floor shop produces correct segments in reverse',
      () async {
        mapBloc.add(const LoadMap());
        await expectLater(mapBloc.stream, emitsThrough(isA<MapLoaded>()));
        ...
        mapBloc.add(
          const CalculateRoute(startNodeId: 'E112', endNodeId: 'E026'),
        );
        await expectLater(
          mapBloc.stream,
          emitsThrough(
    ...
    test(
      'navigation cancellation during floor transition successfully clears session',
      () async {
        mapBloc.add(const LoadMap());
        await expectLater(mapBloc.stream, emitsThrough(isA<MapLoaded>()));
    ...
    test(
      'recalculate route after destination change replaces existing session seamlessly',
      () async {
        mapBloc.add(const LoadMap());
        await expectLater(mapBloc.stream, emitsThrough(isA<MapLoaded>()));
        ...
        mapBloc.add(
          const CalculateRoute(startNodeId: 'E026', endNodeId: 'E027'),
        );
        await expectLater(
          mapBloc.stream,
          emitsThrough(
        ...
        expect(state.navigationSession?.currentSegmentIndex, 0);
        expect(
          state.navigationSession?.segments.length,
          1,
        ); // Single floor routing is 1 segment
      },
    );

    test(
      'dynamic navigation origin resolves correctly from inside a Ground Floor shop',
      () async {
        mapBloc.add(const LoadMap());
        await expectLater(mapBloc.stream, emitsThrough(isA<MapLoaded>()));
        ...
        mapBloc.add(
          const CalculateRoute(startNodeId: 'H001', endNodeId: 'E112'),
        );
        await expectLater(
          mapBloc.stream,
          emitsThrough(
    ...
    test(
      'dynamic navigation origin snaps to nearest node outside shop on First Floor',
      () async {
        mapBloc.add(const LoadMap());
        await expectLater(mapBloc.stream, emitsThrough(isA<MapLoaded>()));
    ...
    test(
      'UpdateUserPosition event to a different floor auto-switches visible floor map',
      () async {
        mapBloc.add(const LoadMap());
        await expectLater(mapBloc.stream, emitsThrough(isA<MapLoaded>()));
        ...
        // Dispatch position on first floor (no active session)
        mapBloc.add(
          UpdateUserPosition(
            IndoorPositionEntity(
              id: 'pos_1',
        ...
        await expectLater(
          ...
          emitsThrough(
            isA<MapLoaded>()
                ...
                .having(
                  (s) => s.latestPosition!.floorId,
                  'latestPosition.floorId',
                  'first_floor',
                ),
          ),
        );
