...
class BrandBloc extends Bloc<BrandEvent, BrandState> {
  ...
  Future<void> _onLoadBrandDetails(
    LoadBrandDetails event,
    Emitter<BrandState> emit,
  ) async {
    emit(const BrandLoading());
    try {
      ...
      final collections = await getBrandCollectionsUseCase.execute(
        event.brandId,
      );
  ...
  void _onSearchBrands(SearchBrands event, Emitter<BrandState> emit) {
    if (state is BrandsLoaded) {
      ...
      emit(
        current.copyWith(searchQuery: event.query, filteredBrands: filtered),
      );
    }
  }

  void _onFilterBrandsByFloor(
    FilterBrandsByFloor event,
    Emitter<BrandState> emit,
  ) {
    if (state is BrandsLoaded) {
      final current = state as BrandsLoaded;
      ...
      emit(
        current.copyWith(selectedFloor: event.floor, filteredBrands: filtered),
      );
    }
  }

  void _onFilterBrandsByCategory(
    FilterBrandsByCategory event,
    Emitter<BrandState> emit,
  ) {
    if (state is BrandsLoaded) {
      final current = state as BrandsLoaded;
  ...
  Future<void> _onToggleFavorite(
    ...
    try {
      ...
      if (event.isDetailsPage && state is BrandDetailsLoaded) {
        ...
        emit(
          current.copyWith(
            brand: current.brand.copyWith(
              isFavorite: !current.brand.isFavorite,
            ),
          ),
        );
    ...
    try {
      ...
      if (state is BrandDetailsLoaded) {
        ...
        emit(
          current.copyWith(
            brand: current.brand.copyWith(
              isFollowed: !current.brand.isFollowed,
            ),
          ),
        );
      }
  ...
  List<BrandEntity> _applyFilters(
    ...
    return brands.where((brand) {
      final matchesFloor =
          floor == 'All Floors' ||
          brand.floor.toLowerCase().contains(floor.toLowerCase());
